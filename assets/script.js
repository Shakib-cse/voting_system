document.addEventListener("DOMContentLoaded", () => {
  ...
  if (voteForm) {
    voteForm.addEventListener("submit", (e) => {
      ...
      const ageCategory = voteAgeCategoryInput
        ? voteAgeCategoryInput.value.trim()
        : "";
  ...
  window.openComicViewer = function (pagesJsonStr) {
    try {
      currentComicPages = JSON.parse(decodeURIComponent(pagesJsonStr)).filter(
        (p) => p !== null && p !== "",
      );
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      ...
      e.key === "ArrowRight" &&
      comicModal.classList.contains("active")
    ) {
      nextComicPage();
    } else if (
      e.key === "ArrowLeft" &&
